...
const Footer: React.FC = () => {
  ...
  const onNavClick = useCallback((e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    e.preventDefault(); smoothScrollTo(id);
  }, [smoothScrollTo]);
  const scrollToTop = useCallback(() => window.scrollTo({ top: 0, behavior: "smooth" }), []);

  const [copied, setCopied] = useState(false);
  const copyPhone = useCallback(async () => {
    try { await navigator.clipboard.writeText(phoneDigits); setCopied(true); setTimeout(() => setCopied(false), 1200); } catch {}
  }, [phoneDigits]);
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-center">
          ...
          <div className="text-center lg:text-left">
            <button type="button" onClick={scrollToTop} aria-label="К началу страницы" className="inline-block mb-6 group">
              <img src={yanaLogo} alt="ЯНА ПИЛИТ — логотип"
          ...
          <div className="text-center">
            <nav className="space-y-4">
              <a href="#about" onClick={(e)=>onNavClick(e,"about")} className="block font-medium text-foreground hover:text-primary transition-colors">ОБО МНЕ</a>
              <a href="#services" onClick={(e)=>onNavClick(e,"services")} className="block font-medium text-foreground hover:text-primary transition-colors">УСЛУГИ</a>
              <a href="#portfolio" onClick={(e)=>onNavClick(e,"portfolio")} className="block font-medium text-foreground hover:text-primary transition-colors">ПОРТФОЛИО</a>
              <a href="#reviews" onClick={(e)=>onNavClick(e,"reviews")} className="block font-medium text-foreground hover:text-primary transition-colors">ОТЗЫВЫ</a>
              <a href="#certificates" onClick={(e)=>onNavClick(e,"certificates")} className="block font-medium text-foreground hover:text-primary transition-colors">СЕРТИФИКАТЫ</a>
            </nav>
          </div>
          ...
          <div className="flex flex-col items-center lg:items-end gap-5 sm:gap-6">
            ...
            {/* кнопка записи — свой ряд, одинаковые отступы сверху/снизу */}
            <div className="w-full flex justify-center lg:justify-end">
              <Button asChild className="btn-hero text-lg px-8 py-3">
                <a href="https://dikidi.ru/1679582" target="_blank" rel="noopener noreferrer">
                  ЗАПИСАТЬСЯ
                </a>
              </Button>
            </div>
